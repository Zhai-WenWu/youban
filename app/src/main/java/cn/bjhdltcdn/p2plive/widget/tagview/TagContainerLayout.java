...
public class TagContainerLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mVerticalInterval = (int) attributes.getDimension(R.styleable.AndroidTagView_vertical_interval,
                TagViewUtils.dp2px(context, DEFAULT_INTERVAL));
        ...
        mTagBorderWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_border_width,
                TagViewUtils.dp2px(context, mTagBorderWidth));
        mTagBorderRadius = attributes.getDimension(
                R.styleable.AndroidTagView_tag_corner_radius, TagViewUtils.dp2px(context, mTagBorderRadius));
        mTagHorizontalPadding = (int) attributes.getDimension(
                R.styleable.AndroidTagView_tag_horizontal_padding,
                TagViewUtils.dp2px(context, mTagHorizontalPadding));
        mTagVerticalPadding = (int) attributes.getDimension(
                R.styleable.AndroidTagView_tag_vertical_padding, TagViewUtils.dp2px(context, mTagVerticalPadding));
        mTagTextSize = attributes.getDimension(R.styleable.AndroidTagView_tag_text_size,
                TagViewUtils.sp2px(context, mTagTextSize));
        ...
        mCrossLineWidth = attributes.getDimension(R.styleable.AndroidTagView_tag_cross_line_width,
                TagViewUtils.dp2px(context, mCrossLineWidth));
...
//        int widthSpecMode = MeasureSpec.getMode(widthMeasureSpec);
    ...
    private void onSetTag() {
        ...
        for (int i = 0; i < mTags.size(); i++) {
            String text = null;
            Object object = mTags.get(i);
            // 添加不同类型的标签对象
            if (object instanceof KeywordInfo) {
                KeywordInfo keywordInfo = (KeywordInfo) object;
                text = keywordInfo.getKeywordName();

            } else if (object instanceof LabelInfo) {
                text = ((LabelInfo) object).getLabelName();
            } else if (object instanceof HobbyInfo) {
                text = ((HobbyInfo) object).getHobbyName();
            }
            else if (object instanceof RefundReason) {
                text = ((RefundReason) object).getReasonName();
            }
            onAddTag(text, mChildViews.size());
        }
        postInvalidate();
    }
    ...
    public void setTagBdDistance(float tagBdDistance) {
        this.mTagBdDistance = TagViewUtils.dp2px(getContext(), tagBdDistance);
    }


    public void setListTags(List tags,boolean isOnSetTag) {
        mTags = tags;
        if (isOnSetTag) {
            onSetTag();
        }
     ...
     * @param tags
     */
    public void setListTags(List tags) {
        mTags = tags;
        onSetTag();
    }
     ...
     */
    public void setTags(List<Object> tags, List<int[]> colorArrayList) {
        mTags = tags;
     ...
     * @param tags
     */
    public void setTagsByObject(Object... tags) {
...
//    }
    ...
    public void setVerticalInterval(float interval) {
        mVerticalInterval = (int) TagViewUtils.dp2px(getContext(), interval);
        postInvalidate();
    }
    ...
    public void setHorizontalInterval(float interval) {
        mHorizontalInterval = (int) TagViewUtils.dp2px(getContext(), interval);
        postInvalidate();
    }
